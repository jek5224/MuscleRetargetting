...
class SKELOutput(ModelOutput):
    ...
    joints_orig: Optional[Tensor] = None
...
class SKEL(nn.Module):
    ...
    def forward(self, poses, betas, trans, poses_type='skel', skelmesh=True, dJ=None, pose_dep_bs=True):      
        ...
        '''
        What would be joint translations without vertebrae and scapula displacement?
        '''
        results_orig = [root_transform]
        for i in range(0, self.parent.shape[0]):
            transform_i =  with_zeros(torch.cat((R[:, i + 1], t[:,i+1]), 2))
            curr_res = torch.matmul(results_orig[self.parent[i]],transform_i)
            results_orig.append(curr_res)
        results_orig = torch.stack(results_orig, dim=1)
        posed_joints_orig = results_orig[:, :, :3, 3]
        J_orig = posed_joints_orig + trans[:,None,:]
