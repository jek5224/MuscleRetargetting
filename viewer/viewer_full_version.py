...
class GLFWApp():
    ...
    def setEnv(self, env):  
        ...
        zygote_dir = 'Zygote_Meshes_Revised/'
        for muscle_file in os.listdir(zygote_dir + 'Muscle'):
            if muscle_file.endswith('.obj'):
                ...
                self.zygote_muscle_meshes[muscle_name].find_geodesic_reference_line()  # Muscle-specific
    ...
    def update(self):
        ...
        for muscle_name, muscle in self.zygote_muscle_meshes.items():
            if muscle.tet is not None:
                muscle.tet.update(1 / self.env.controlHz)
            ...
            if imgui.tree_node("Muscle"):                
                ...
                for name, obj in self.zygote_muscle_meshes.items():
                    if imgui.tree_node(name):
                        ...
                        if imgui.button(f"Find Contours##{name}", width=wide_button_width):
                            obj.find_contours()
                        if imgui.button(f"Trim Contours##{name}", width=wide_button_width):
                            obj.trim_contours()
                        if imgui.button(f"Smoothen Contours##{name}", width=wide_button_width):
                            obj.smoothen_contours()
                        if imgui.button(f"Find Streams##{name}", width=wide_button_width):
                            obj.find_contour_stream()
                        _, obj.is_one_fiber = imgui.checkbox(f"One Fiber##{name}", obj.is_one_fiber)

                        if imgui.button(f"Set Body Nodes##{name}", width=wide_button_width):
                            if obj.tet is not None and len(obj.draw_contour_stream) == 1:
                                obj.tet.set_body_nodes([self.env.skel.getBodyNode(obj.attach_skeletons[0][0] + obj.attach_skeletons_sub[0][0]),
                                                        self.env.skel.getBodyNode(obj.attach_skeletons[0][1] + obj.attach_skeletons_sub[0][1])])
                        ...
                        _, obj.is_draw_tet = imgui.checkbox("Draw Tetrahedra", obj.is_draw_tet)
