...
class GLFWApp():
        ...
        if action == glfw.PRESS:
            ...
            if button == glfw.MOUSE_BUTTON_MIDDLE:
                ...
                if self.paintingMode:
                    ...
                    if (self.isOrigin or self.isInsertion) and selected_face is not None:
                        if self.isOrigin:
                            painted_faces = self.painted_origins
                        else:
                            painted_faces = self.painted_insertions

                        if not self.erasingMode:
                            if not selected_mesh_index in painted_faces.keys():
                                painted_faces[selected_mesh_index] = []
                                painted_faces[selected_mesh_index].append(selected_face)
                            else:
                                if not selected_face in painted_faces[selected_mesh_index]:
                                    painted_faces[selected_mesh_index].append(selected_face)
                        else:
                            if selected_mesh_index in painted_faces.keys():
                                if selected_face in painted_faces[selected_mesh_index]:
                                    painted_faces[selected_mesh_index].remove(selected_face)
    ...
    def mouseMove(self, xpos, ypos):
        ...
        # if self.paintingMode and self.muscleMode:
        #     xpos, ypos = glfw.get_cursor_pos(self.window)
        #     selected_face, selected_point, selected_mesh_index = self.pick_face(xpos, ypos, self.width, self.height)
        #     self.selected_mesh_index = selected_mesh_index
        ...
        elif self.selected_origin is not None and self.selected_insertion is not None:
            if self.isWaypoint:  
                ray_origin, ray_direction = self.get_ray_from_cursor(xpos, ypos, self.width, self.height)
    ...
    def drawUIFrame(self):
        ...
        if imgui.tree_node("Muscle GUI"):
            _, self.paintingMode = imgui.checkbox("Painting Mode", self.paintingMode)
            if self.paintingMode:
                _, self.erasingMode = imgui.checkbox("Erasing Mode", self.erasingMode)
            if imgui.button("Clear painted insertions"):
                self.painted_insertions = {}
            imgui.same_line()
            if imgui.button("Clear painted origins"):
                self.painted_origins = {}
