...
class ContourMeshMixin(ContourAnimationMixin):
    ...
    def smoothen_contours_z(self):
        ...
        def _fix_handedness(bp):
            """Recompute basis_y = cross(z, x) to maintain right-handedness after z-flip."""
            bp['basis_y'] = np.cross(bp['basis_z'], bp['basis_x'])
    ...
    def select_stream_levels(self, error_threshold=None):
        ...
        if error_threshold is None:
            error_threshold = getattr(self, 'level_select_error_threshold', 0.05) * muscle_length
