...
from viewer.contour_animation import ContourAnimationMixin


class ContourMeshMixin(ContourAnimationMixin):
    """
    ...
    Animation replay/update methods are in ContourAnimationMixin (contour_animation.py).
    """
    ...
    def _init_contour_properties(self):
        ...
        # Animation state (replay, update, snapshots) — see contour_animation.py
        self._init_animation_properties()
    ...
    def smoothen_all(self, defer=False):
        ...
        self._smooth_swing_data = smooth_swing_data
        self._smooth_twist_data = smooth_twist_data
    ...
    def smoothen_contours_z(self):
        """
        Align z-axes to point from origin (level 0) toward insertion (last level).

        For each contour at each level, computes the local direction toward
        insertion and flips z if it points the wrong way.  This is simple,
        robust, and works regardless of contour count changes between levels.
        """
        if len(self.bounding_planes) < 2:
        ...
        for stream_i in range(num_streams):
            ...
            if forward_norm > 1e-10:
                ...
                if dot_z_forward < 0:
                    ...
                    old_x = bp_stream[0]['basis_x'].copy()
                    new_z = bp_stream[0]['basis_z']
                    new_x = old_x - np.dot(old_x, new_z) * new_z
                    nxn = np.linalg.norm(new_x)
                    bp_stream[0]['basis_x'] = new_x / nxn if nxn > 1e-10 else old_x
                    bp_stream[0]['basis_y'] = np.cross(new_z, bp_stream[0]['basis_x'])
            ...
            for level in range(1, stream_len):
                ...
                if dot_z < 0:
                    ...
                    old_x = bp_stream[level]['basis_x'].copy()
                    new_z = bp_stream[level]['basis_z']
                    new_x = old_x - np.dot(old_x, new_z) * new_z
                    nxn = np.linalg.norm(new_x)
                    bp_stream[level]['basis_x'] = new_x / nxn if nxn > 1e-10 else old_x
                    bp_stream[level]['basis_y'] = np.cross(new_z, bp_stream[level]['basis_x'])
    ...
    def smoothen_contours_x(self):
        ...
        for contour_idx in range(contour_counts[start_level]):
            ...
            best_x, best_y, best_angle = self._best_4rotation_projected(curr_bp, ref_x)
        ...
        for level_idx in range(start_level + 1, num_levels):
            ...
            for contour_idx in range(curr_count):
                ...
                best_x, best_y, best_angle = self._best_4rotation_projected(curr_bp, prev_bp['basis_x'])
        ...
        for level_idx in range(start_level - 1, -1, -1):
            ...
            for contour_idx in range(curr_count):
                ...
                best_x, best_y, best_angle = self._best_4rotation_projected(curr_bp, next_bp['basis_x'])
    ...
    @staticmethod
    def _best_4rotation_projected(curr_bp, ref_x_3d):
        """Pick 0/90/180/270° rotation of curr x/y that best matches ref_x
        projected onto curr's plane. Ignores z-axis differences.
        Returns (best_x, best_y, best_angle)."""
        curr_x = curr_bp['basis_x']
        curr_y = curr_bp['basis_y']
        curr_z = curr_bp['basis_z']

        ref_proj = ref_x_3d - np.dot(ref_x_3d, curr_z) * curr_z
        ref_proj_norm = np.linalg.norm(ref_proj)
        if ref_proj_norm < 1e-10:
            return curr_x, curr_y, 0
        ref_proj = ref_proj / ref_proj_norm
        ...
        best_dot = -2.0
        best_x, best_y, best_angle = curr_x, curr_y, 0
        for cx, cy, angle in candidates:
            d = np.dot(cx, ref_proj)
            if d > best_dot:
                best_dot = d
    ...
    def _smoothen_contours_x_stream_mode(self):
        ...
        for stream_i in range(num_streams):
            ...
            if len(non_sq) >= 2:
                ...
                best_x, best_y, best_angle = self._best_4rotation_projected(
                    first_bp, np.array([1.0, 0.0, 0.0]))
                if best_angle != 0:
    ...
    def _resample_contour_with_endpoints(self, contour, endpoint_indices, num_samples, new_shared_cut_vertices):
        ...
        for ep in [endpoint1, endpoint2]:
            already_added = False
            for sv in new_shared_cut_vertices:
                if np.linalg.norm(ep - sv) < eps:
                    already_added = True
                    break
    ...
    def _optimize_remaining_pieces(self, current_pieces_3d, source_contours, source_bps,
        ...
        for local_idx, assigned_sources in enumerate(piece_to_sources):
            ...
            if len(assigned_sources) == 0:
                ...
                # Respect is_common_mode from manual cut data
                ...
                sub_pieces, _ = self._cut_contour_bp_transform(
                    ...
                    is_first_division=is_first_div,
    ...
    def _cut_contour_bp_transform(self, target_contour, target_bp, source_contours, source_bps, stream_indices, is_first_division=True, target_level=None, source_level=None):
        ...
        if use_intersection_method and all(p is not None for p in intersection_pieces_2d):
            ...
            print(f"  [BP Transform] {len(all_crossings)} crossings for {n_pieces} pieces")
            for i, c in enumerate(all_crossings):
                print(f"    Crossing {i}: edge {c[0]}, t={c[1]:.3f}, pair={c[4]}")

            if n_pieces == 2:
                # Simple 2-piece case: 2 crossings split contour into 2 parts
                cut1 = all_crossings[0]
                ...
                # N-piece case: Use DIVIDE AND CONQUER with iterative 2-piece cuts
                # Each cut separates one source from the rest using their shared boundary

                print(f"  [BP Transform] N-piece cut using DIVIDE AND CONQUER")
                print(f"  [BP Transform] Assignment counts: {[assignments.count(i) for i in range(n_pieces)]}")

                # Build adjacency map from shared edges
                adjacency = {i: set() for i in range(n_pieces)}
                shared_edge_map = {}  # (i, j) -> shared_pts
                for pair_indices, shared_pts in shared_edges:
                    i, j = pair_indices
                    adjacency[i].add(j)
                    adjacency[j].add(i)
                ...
                source_to_piece = {}  # source_idx -> piece vertices
                ...
                for cut_idx, (src_to_cut, adjacent_src) in enumerate(cut_order):
                    ...
                    if pair_key not in shared_edge_map:
                        print(f"  [BP Transform] WARNING: No shared edge for pair {pair_key}")
                        continue
                    ...
                    if len(shared_pts) >= 2:
                        ...
                        line_point = np.mean(shared_pts_arr, axis=0)
                        if len(shared_pts) >= 2:
                            _, _, vh = np.linalg.svd(shared_pts_arr - line_point)
                            line_dir = vh[0]
                        else:
                            line_dir = shared_pts[-1] - shared_pts[0]
                            line_dir = line_dir / np.linalg.norm(line_dir)
                    ...
                    current_crossings.sort(key=lambda c: c[0] + c[1])
                    cut1 = current_crossings[0]
                    cut2 = current_crossings[1]

                    edge1, t1, pt2d_1, pt3d_1 = cut1
                    edge2, t2, pt2d_2, pt3d_2 = cut2

                    # Build two pieces
                    piece_a = [pt3d_1]
                    piece_a_2d = [pt2d_1]
                    v = (edge1 + 1) % n_current
                    while v != (edge2 + 1) % n_current:
                        piece_a.append(current_target[v])
                        piece_a_2d.append(current_target_2d[v])
                        v = (v + 1) % n_current
                    piece_a.append(pt3d_2)
                    piece_a_2d.append(pt2d_2)

                    piece_b = [pt3d_2]
                    piece_b_2d = [pt2d_2]
                    v = (edge2 + 1) % n_current
                    while v != (edge1 + 1) % n_current:
                        piece_b.append(current_target[v])
                        piece_b_2d.append(current_target_2d[v])
                        v = (v + 1) % n_current
                ...
                last_source = list(remaining_sources)[0]
                source_to_piece[last_source] = current_target
