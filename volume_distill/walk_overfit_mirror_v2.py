...
V2 uses PCA output basis, temporal consistency loss, 20D derivative features,
muscle embedding + single shared decoder, and linear baseline + residual.
...
def build_derivative_features(dofs):
    """Build 20D features: [q_t, dq_t, ddq_t, q_{t-1}, q_{t-2}] from (N, 4) DOFs.

    Returns (N, 20) feature array.
    """
    N = dofs.shape[0]
    q_t = dofs                                              # (N, 4)
    q_prev1 = np.zeros_like(dofs)
    q_prev1[1:] = dofs[:-1]
    q_prev1[0] = dofs[0]
    q_prev2 = np.zeros_like(dofs)
    q_prev2[2:] = dofs[:-2]
    q_prev2[1] = dofs[0]
    q_prev2[0] = dofs[0]

    dq = q_t - q_prev1                                     # (N, 4)
    ddq = q_t - 2 * q_prev1 + q_prev2                      # (N, 4)

    return np.concatenate([q_t, dq, ddq, q_prev1, q_prev2], axis=1)  # (N, 20)


class TemporalDataset(Dataset):
    """Dataset returning frame pairs (t, prev) for temporal consistency loss."""
    def __init__(self, input_dofs, prev_frame_idx, pca_targets, muscle_names, indices):
...
def temporal_collate_fn(batch):
    ...
    x_t = torch.stack(x_ts)
    x_prev = torch.stack(x_prevs)
    x_combined = torch.cat([x_t, x_prev], dim=0)
    names = list(tgt_ts[0].keys())
    targets_t = {name: torch.stack([t[name] for t in tgt_ts]) for name in names}
    targets_prev = {name: torch.stack([t[name] for t in tgt_prevs]) for name in names}
...
def preprocess():
    ...
    # Build 20D derivative features for L and R
    ...
    l_features = build_derivative_features(l_dofs)    # (N, 20)
    r_features = build_derivative_features(r_dofs_mirrored)  # (N, 20)
    input_dofs = torch.from_numpy(
        np.concatenate([l_features, r_features], axis=0).astype(np.float32)
    )  # (2N, 20)

    total = input_dofs.shape[0]

    # prev_frame_idx: frame t-1 for temporal loss (boundary → self)
    prev_frame_idx = torch.arange(total)
    prev_frame_idx[1:N] = torch.arange(N - 1)       # L side: frame i → frame i-1
    prev_frame_idx[0] = 0                             # L boundary
    prev_frame_idx[N + 1:] = torch.arange(N, 2 * N - 1)  # R side: frame i → frame i-1
    prev_frame_idx[N] = N                             # R boundary
    ...
    return (input_dofs, prev_frame_idx, pca_targets, pca_components, pca_means, pca_stds,
...
def train():
    ...
    (input_dofs, prev_frame_idx, pca_targets, pca_components, pca_means, pca_stds,
    ...
    ds = TemporalDataset(input_dofs, prev_frame_idx, pca_targets, muscle_names, indices)
    loader = DataLoader(ds, batch_size=BATCH_SIZE, shuffle=True, collate_fn=temporal_collate_fn)
    ...
    for epoch in range(start_epoch + 1, end_epoch + 1):
        ...
        for x_combined, targets_t, targets_prev in loader:
            ...
            for m_idx in range(num_muscles):
                ...
                pred_t = pred_combined[:B]
                pred_prev = pred_combined[B:]
                gt_t = targets_t[name]
                gt_prev = targets_prev[name]
        ...
        avg_temporal = epoch_temporal / n_batches
        avg_loss = avg_recon + TEMPORAL_LOSS_WEIGHT * avg_temporal
        lr = optimizer.param_groups[0]["lr"]
        elapsed = time.time() - t0

        writer.add_scalar("loss/recon", avg_recon, epoch)
        writer.add_scalar("loss/temporal", avg_temporal, epoch)
        writer.add_scalar("loss/total", avg_loss, epoch)
        writer.add_scalar("lr", lr, epoch)

        if epoch % 100 == 0 or epoch <= start_epoch + 5:
            print(f"Epoch {epoch:5d}/{end_epoch} | Recon: {avg_recon:.2e} | Temp: {avg_temporal:.2e} | "
                  f"Total: {avg_loss:.2e} | LR: {lr:.2e} | {elapsed:.1f}s")
