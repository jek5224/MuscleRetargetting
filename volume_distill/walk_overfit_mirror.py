"""Train V1 DistillNet on walk.bvh with L+R UpLeg data (mirrored to L-canonical).

L side: used directly. R side: DOFs mirrored (negate hip X), vertices mirrored (negate X).
This doubles the training data and produces a network that operates in L-canonical space.
At inference, R side DOFs are mirrored to L, predicted, then output mirrored back.

Usage: python -m volume_distill.walk_overfit_mirror
"""
import os
import glob
import time
...
def load_side(cache_dir, prefix, mocap, ref_R, ref_t, N):
    ...
    for npz_path in npz_files:
        ...
        # Use L_ canonical name for all muscles
        ...
        if prefix == "R":
            # Mirror R positions to L-canonical: negate X
            local_pos[:, :, 0] *= -1
...
def preprocess():
    """Load L+R walk cache, mirror R to L-canonical space."""
    print("=== Preprocessing walk.bvh (L+R mirrored) ===")
    ...
    muscle_names = l_names  # Both sides should produce same canonical names
    rest_positions = {}
    displacements = {}
    ...
    input_dofs = torch.from_numpy(np.concatenate([l_dofs, r_dofs_mirrored], axis=0))  # (2N, 4)
...
def train():
    ...
    run_name = "walk_mirror_" + datetime.now().strftime("%Y%m%d_%H%M%S")
