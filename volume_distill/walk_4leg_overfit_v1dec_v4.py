...
Usage: python -m volume_distill.walk_4leg_overfit_v1dec
"""
import os
import glob
import time
...
CHECKPOINT_DIR = "volume_distill/walk_v1dec_v4_checkpoints"
LOG_DIR = "volume_distill/walk_v1dec_v4_runs"
...
NUM_ENCODER_RES = 4
NUM_DECODER_RES = 4
TEMPORAL_LAMBDA = 0.5  # weight on adjacent-frame delta-MSE
ACCEL_LAMBDA = 0.25    # weight on adjacent-frame acceleration-MSE (2nd order)
...
def train():
    ...
    # Full-batch training: stack all once, move to device
    x_all = input_dofs.to(device)
    targets_all = {name: displacements[name].reshape(displacements[name].shape[0], -1).to(device)
                   for name in muscle_names}
    n_total = x_all.shape[0]
    # Layout: first half = L frames (sorted), second half = R frames (mirrored, sorted).
    # Build adjacent-frame pair indices within each side.
    if n_total % 2 == 0:
        n_per_side = n_total // 2
        sides = [(0, n_per_side), (n_per_side, n_total)]
    else:
        sides = [(0, n_total)]
    pair_i, pair_j = [], []
    ...
    for (s, e) in sides:
        ...
        for i in range(s, e - 2):
            ...
            trip_j.append(i + 1)
            trip_k.append(i + 2)
    pair_i = torch.tensor(pair_i, dtype=torch.long, device=device)
    pair_j = torch.tensor(pair_j, dtype=torch.long, device=device)
    trip_i = torch.tensor(trip_i, dtype=torch.long, device=device)
    trip_j = torch.tensor(trip_j, dtype=torch.long, device=device)
    trip_k = torch.tensor(trip_k, dtype=torch.long, device=device)
    print(f"\nTrain: {n_total} samples (full-batch), {len(muscle_names)} muscles, "
          f"{pair_i.shape[0]} adjacent pairs, {trip_i.shape[0]} accel triplets, "
          f"lambda_t={TEMPORAL_LAMBDA}, lambda_a={ACCEL_LAMBDA}")
    ...
    def lr_at(ep):
        if ep <= WARMUP_EPOCHS:
            return LR * ep / max(1, WARMUP_EPOCHS)
        prog = (ep - WARMUP_EPOCHS) / max(1, EPOCHS - WARMUP_EPOCHS)
        prog = min(1.0, max(0.0, prog))
        return LR_MIN + 0.5 * (LR - LR_MIN) * (1 + math.cos(math.pi * prog))
    scheduler = None
    ...
    for epoch in range(start_epoch + 1, end_epoch + 1):
        ...
        preds = model(x_all)
        data_loss = sum(
            ((preds[name] - targets_all[name]) ** 2).mean()
            for name in muscle_names
        ) / len(muscle_names)
        delta_loss = sum(
            (((preds[name][pair_j] - preds[name][pair_i])
              - (targets_all[name][pair_j] - targets_all[name][pair_i])) ** 2).mean()
            for name in muscle_names
        ) / len(muscle_names)
        accel_loss = sum(
            (((preds[name][trip_k] - 2 * preds[name][trip_j] + preds[name][trip_i])
              - (targets_all[name][trip_k] - 2 * targets_all[name][trip_j] + targets_all[name][trip_i])) ** 2).mean()
            for name in muscle_names
        ) / len(muscle_names)
        loss = data_loss + TEMPORAL_LAMBDA * delta_loss + ACCEL_LAMBDA * accel_loss
        ...
        avg_loss = data_loss.item()  # report raw data MSE for comparison vs v1/v2
        new_lr = lr_at(epoch)
        ...
        writer.add_scalar("loss/delta", delta_loss.item(), epoch)
        writer.add_scalar("loss/accel", accel_loss.item(), epoch)
        ...
        if epoch % 100 == 0 and pending_save:
            torch.save({
                "epoch": epoch,
