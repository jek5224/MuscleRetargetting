"""Preprocess walk.bvh cache + train V1 model to overfit.

132 frames × 25 muscles. Input: 4 raw DOFs only (deterministic mapping).
No constraint losses — in pelvis-local frame, fixed vertices move with bones
so penalizing their displacement is incorrect.

Usage: python -m volume_distill.walk_overfit
"""
import os
import glob
import time
...
CACHE_DIR = "data/motion_cache/walk"
CHECKPOINT_DIR = "volume_distill/walk_checkpoints"
LOG_DIR = "volume_distill/walk_runs"
...
def preprocess():
    """Convert walk cache → input DOFs + pelvis-local displacements."""
    print("=== Preprocessing walk.bvh ===")
    ...
    # Process muscles
    npz_files = sorted(glob.glob(os.path.join(CACHE_DIR, "L_*_chunk_0000.npz")))
    muscle_names = []
    rest_positions = {}
    displacements = {}
    ...
    for npz_path in npz_files:
        ...
        print(f"  {mname}: {positions.shape[1]} verts, disp range [{disp.min():.4f}, {disp.max():.4f}]")
...
def train():
    ...
    run_name = "walk_overfit_" + datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    for epoch in range(start_epoch + 1, end_epoch + 1):
        ...
        if epoch % 50 == 0 or epoch <= start_epoch + 5:
            print(f"Epoch {epoch:4d}/{end_epoch} | MSE: {avg_loss:.2e} | LR: {lr:.2e} | {elapsed:.1f}s")
