#!/bin/bash
# Run moment-arm analysis on every R muscle across both leg regions.
set -e
...
for M in "${UPLEG[@]}"; do
    python3 tools/fiber_moment_arm_analyze.py --muscle "$M" \
        --cache-dir data/motion_cache/walk/R_UpLeg --clusters 8 2>&1 \
        | grep -E "^STATS|Saved|fibers|stream" || true
done
for M in "${LOWLEG[@]}"; do
    python3 tools/fiber_moment_arm_analyze.py --muscle "$M" \
        --cache-dir data/motion_cache/walk/R_LowLeg --clusters 8 2>&1 \
        | grep -E "^STATS|Saved|fibers|stream" || true
done
